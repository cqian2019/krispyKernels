...
def add():
    ...
    event = api.getEvent(eventId)
    # print(3)
    location = api.getEventLocation(event)
    # print(endStr)
    directions = api.drivingDir(startStr, location)
    # print(4)
    url = session['eventInfo'][5]
    db.addEvent(username, eventName, date, location, directions, url)
    return redirect('/')
@app.route('/view', methods=["GET","POST"])
def view():
    user = (session['username'])
    return render_template('saved.html', info=db.displayEvents(user))
